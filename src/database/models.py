...
class User(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    phone_number = mapped_column(String(255), index=True, nullable=False)
    status: Mapped[str] = mapped_column(Enum(UserStatus, name="user_status"), server_default=UserStatus.NOT_VERIFIED.value)
    created_at: Mapped[DateTime] = mapped_column(DateTime(), server_default=func.now())
